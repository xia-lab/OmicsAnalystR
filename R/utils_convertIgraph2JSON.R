# Utility functions from OmicsNet for data set management
...
my.convert.igraph <- function(dataSetObj=NA, net.nm, filenm, thera=FALSE, dim=3){
  ...
  exp <-as.vector(vertex_attr(g, name="abundance", index = V(g)));
  exp[is.na(exp)] <- 0;
...
if (length(unique(lens)) == 1 && lens[1] > 0) {
  ...
  # Add metadata fields required by network viewer JavaScript
  if(exists("data.org")) {
    netData[["org"]] <- data.org;
  } else {
    netData[["org"]] <- "NA";
  }
  netData[["analType"]] <- "network";  # For network analysis
  netData[["tblNm"]] <- net.nm;  # Network/table name
  netData[["naviString"]] <- "NA";  # Navigation string
